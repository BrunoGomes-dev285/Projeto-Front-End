...
const infraestruturaPadrao = [
  { id: 1, nome: 'Link VSAT Principal', tipo: 'Satélite Starlink / LEO', status: 'Ativo' },
  { id: 2, nome: 'Link VSAT BGAN', tipo: 'Satélite Inmarsat Backup', status: 'Ativo' },
  { id: 3, nome: 'Roteamento OSPF', tipo: 'Rede Core Interna', status: 'Ativo' },
  { id: 4, nome: 'Sessão BGP', tipo: 'Link Operadora / ISP', status: 'Ativo' },
  { id: 5, nome: 'LTE-Móvel', tipo: 'Conexão Celular 4G/5G', status: 'Ativo' }
];

function DashboardRouter() {
  const [dados, setDados] = useState({ 
    infraestrutura: infraestruturaPadrao, 
    frota: [], 
    noc: { latitude: -23.5258, longitude: -46.7348 } 
  });
  ...
  const carregarDados = () => {
    fetch('http://localhost:3000/api/frota')
      .then((response) => response.json())
      .then((data) => { 
        setDados((prev) => ({ 
          ...prev, 
          frota: Array.isArray(data) ? data : (data.frota || []),
          infraestrutura: (data && data.infraestrutura && data.infraestrutura.length > 0) 
            ? data.infraestrutura 
            : infraestruturaPadrao
        })); 
        setCarregando(false); 
      })
      .catch((error) => { 
        ...
        <div className="container-fluid flex-column align-items-start px-3 py-2">
          <div className="d-flex w-100 justify-content-between align-items-center mb-3">
            ...
            <div className="d-flex align-items-center gap-2">
              ...
              >
                <i className="bi bi-database-gear me-1"></i> GERENCIAR FROTA (CRUD)
              </button>
          ...
          <div className="nav-scroll w-100 gap-2">
            ...
            {categoriasVeiculos.map((cat) => (
              <Link 
                ...
                to={`/frota/${cat}`} 
                onClick={() => setTempoRestante(5)} 
                className={`btn btn-sm text-nowrap px-3 py-2 ${decodeURIComponent(location.pathname) === `/frota/${cat}` ? 'btn-light text-dark fw-bold shadow' : 'btn-outline-light text-white'}`}
              >
                {cat}
              </Link>
            ))}
